...
func merge* [SA; A; SB; B](
  self: Stream[SA, A];
  other: Stream[SB, B]
): Stream[Pair[SA, SB], Pair[A, B]] =
  ##[
    Combines the items generated by `self` with the ones by `other` in a single
    stream.

    If one of the stream stops generating items before the other, the returned
    stream will stop too.
  ...
  self
    ...
    .merge(other.loop)
    .startingAt(
      () => pair(self.initialStep.run(), other.initialStep.run()),
      (pair: Pair[SA, SB]) =>
        pair.apply(self.onCloseEvent, other.onCloseEvent).reduce((u, _) => u)
    )
